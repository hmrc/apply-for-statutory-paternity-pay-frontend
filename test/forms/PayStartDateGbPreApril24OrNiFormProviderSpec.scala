...
class PayStartDateGbPreApril24OrNiFormProviderSpec extends DateBehaviours {
  ...
  val form = new PayStartDateGbPreApril24OrNiFormProvider()(limits)
  ...
  ".value" - {
    ...
    behave like dateFieldWithMax(
      ...
      "value",
      limits.max,
      FormError("value", "payStartDateGbPreApril24OrNi.error.tooHigh", Seq(limits.max.format(dateTimeFormat)))
    )
    ...
    behave like dateFieldWithMin(
      ...
      "value",
      limits.min,
      FormError("value", "payStartDateGbPreApril24OrNi.error.tooLow", Seq(limits.min.format(dateTimeFormat)))
    )
  }
}
