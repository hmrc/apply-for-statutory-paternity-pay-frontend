...
class PayStartDateWeek2FormProviderSpec extends DateBehaviours {
  ...
  val form = new PayStartDateWeek2FormProvider()(limits)
  ...
  ".value" - {
    ...
    behave like dateFieldWithMax(
      ...
      "value",
      limits.max,
      FormError("value", "payStartDateWeek2.error.tooHigh", Seq(limits.max.format(dateTimeFormat)))
    )
    ...
    behave like dateFieldWithMin(
      ...
      "value",
      limits.min,
      FormError("value", "payStartDateWeek2.error.tooLow", Seq(limits.min.format(dateTimeFormat)))
    )
  }
}
