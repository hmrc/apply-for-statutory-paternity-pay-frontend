...
class WillTakeTimeToSupportMotherPageSpec extends PageBehaviours {
  ...
  "WillTakeTimeToSupportMotherPage" - {
    ...
    "must remove all subsequent answers when the answer is no" in {
      ...
      val result = answers.set(WillTakeTimeToSupportMotherPage, false).success.value
      ...
      result.get(WillTakeTimeToCareForChildPage)   must be(defined)
      result.get(WillTakeTimeToSupportMotherPage)  must be(defined)
