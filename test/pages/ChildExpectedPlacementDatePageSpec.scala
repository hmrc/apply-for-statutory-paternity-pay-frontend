...
class ChildExpectedPlacementDatePageSpec extends PageBehaviours with ScalaCheckPropertyChecks with Generators {

  "ChildExpectedPlacementDatePage" - {

    beRetrievable[LocalDate](ChildExpectedPlacementDatePage)

    beSettable[LocalDate](ChildExpectedPlacementDatePage)

    beRemovable[LocalDate](ChildExpectedPlacementDatePage)
    ...
    "when the date is before 7th April 2024" - {
      ...
      "must remove all paternity questions for GB post April" in {
        ...
        forAll(datesBetween(dateBeforeLegislation, Constants.april24LegislationEffective)) { date =>
          ...
          val result = paternityPageAnswers().set(ChildExpectedPlacementDatePage, date).success.value
      ...
      "and the user is in Northern Ireland" - {
        ...
        "must remove all paternity questions for GB post April or NI" in {
          ...
          forAll(datesBetween(Constants.april24LegislationEffective, dateAfterLegislation)) { date =>
            ...
            val result = paternityPageAnswers(NorthernIreland).set(ChildExpectedPlacementDatePage, date).success.value
      ...
      "and the user is in England, Scotland or Wales" - {
        ...
        "must remove all paternity questions for GB post April or NI" in {
          ...
          forAll(datesBetween(Constants.april24LegislationEffective, dateAfterLegislation), Gen.oneOf(England, Scotland, Wales)) { case (date, country) =>
            ...
            val result = paternityPageAnswers(country).set(ChildExpectedPlacementDatePage, date).success.value
