...
import views.html.LeaveTakenTogetherOrSeparatelyView

import scala.concurrent.Future

class LeaveTakenTogetherOrSeparatelyControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  lazy val leaveTakenTogetherOrSeparatelyRoute = routes.LeaveTakenTogetherOrSeparatelyController.onPageLoad(NormalMode).url
  ...
  "LeaveTakenTogetherOrSeparately Controller" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, leaveTakenTogetherOrSeparatelyRoute)
            .withFormUrlEncodedBody(("value", LeaveTakenTogetherOrSeparately.values.head.toString))
