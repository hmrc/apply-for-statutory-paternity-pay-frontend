...
class PayStartDateWeek2ControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val payStartDateWeek2Route = routes.PayStartDateWeek2Controller.onPageLoad(NormalMode).url

  override val emptyUserAnswers = UserAnswers(userAnswersId)

  def getRequest(): FakeRequest[AnyContentAsEmpty.type] =
    FakeRequest(GET, payStartDateWeek2Route)
  ...
  "PayStartDateWeek2 Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(PayStartDateWeek2Page, validAnswer).success.value
