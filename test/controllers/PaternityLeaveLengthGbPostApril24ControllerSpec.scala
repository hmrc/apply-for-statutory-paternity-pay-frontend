...
class PaternityLeaveLengthGbPostApril24ControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  lazy val paternityLeaveLengthGbPostApril24Route = routes.PaternityLeaveLengthGbPostApril24Controller.onPageLoad(NormalMode).url
  ...
  "PaternityLeaveLengthGbPostApril24 Controller" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, paternityLeaveLengthGbPostApril24Route)
            .withFormUrlEncodedBody(("value", PaternityLeaveLengthGbPostApril24.values.head.toString))
