...
class BabyDateOfBirthController @Inject()(
                                        ...
                                        formProvider: BabyDateOfBirthFormProvider,
                                        val controllerComponents: MessagesControllerComponents,
                                        view: BabyDateOfBirthView,
                                        clock: Clock
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(BabyDateOfBirthPage) match {
        case None => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(BabyDateOfBirthPage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(BabyDateOfBirthPage, mode, updatedAnswers))
      )
  }
}
