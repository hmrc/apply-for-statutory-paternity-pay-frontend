...
object PayStartDateWeek1Summary  {

  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(PayStartDateWeek1Page).map {
      _.map {
        answer =>
  ...
  private def viewModel(value: Value)(implicit messages: Messages): SummaryListRow =
    SummaryListRowViewModel(
      key = "payStartDateWeek1.checkYourAnswersLabel",
      value = value,
      actions = Seq(
        ActionItemViewModel("site.change", routes.PayStartDateWeek1Controller.onPageLoad(CheckMode).url)
          .withVisuallyHiddenText(messages("payStartDateWeek1.change.hidden"))
      )
    )
}