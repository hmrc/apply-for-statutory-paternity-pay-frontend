...
sealed trait PaternityLeaveLengthGbPostApril24
...
object PaternityLeaveLengthGbPostApril24 extends Enumerable.Implicits {
  ...
  def options(implicit messages: Messages): Seq[RadioItem] = values.zipWithIndex.map {
    case (value, index) =>
      RadioItem(
        content = Text(messages(s"paternityLeaveLengthGbPostApril24.${value.toString}")),
        value   = Some(value.toString),
        id      = Some(s"value_$index")
      )
  }

  implicit val enumerable: Enumerable[PaternityLeaveLengthGbPostApril24] =
